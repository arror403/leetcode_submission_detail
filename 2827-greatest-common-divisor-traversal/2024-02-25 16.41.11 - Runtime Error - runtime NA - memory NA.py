class Solution:
    def canTraverseAllPairs(self, nums: List[int]) -> bool:
        ...
        d={k:self.prime_factors(k) for k in Counter(nums).keys()}
        ...
        # Artificial connection between the first and last elements if they share a prime number
        first_primes = d[nums[0]]
        last_primes = d[nums[-1]]
        for prime in first_primes:
            if prime in last_primes:
                prime_neighbors[prime].add(L-1)
                prime_neighbors[prime].add(0)

        print(prime_neighbors)
